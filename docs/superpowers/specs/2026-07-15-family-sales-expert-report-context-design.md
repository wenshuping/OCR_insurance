...
- `not_identified`：现有资料未识别到，不能断言没有。
- `conflicted`：不同来源结果冲突。
...
- `needs_verification`：指标未识别或来源不足，不能确认是否存在。
...
```json
{
  ...
  "interpretation": "当前资料未识别到相关责任，不等于确认没有"
}
```
...
- 模型把未识别断言为没有：输出质量门拦截；能确定性改写时改写，否则任务失败。
...
- `0`、`null`、`not_identified`、`conflicted` 和 `missing_source` 正确区分。
