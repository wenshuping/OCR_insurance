...
test('publish readiness requires a bound usable chunk and isolates unbound siblings', () => {
  ...
  const awaitingCorrectionConfirmation = assessProductPublishReadiness({
    document: { payload: {
      ...
      pageReviews: {
        'candidate-v2:1': { pageNo: 1, indexVersion: 'candidate-v2', status: 'pending_confirmation' },
      },
    } },
    links: [{ canonicalProductId: 'product-1', pageStart: 1, pageEnd: 1 }],
    chunks: [base],
  ...
  assert.ok(awaitingCorrectionConfirmation.blockingReasons
    .some((item) => item.code === 'page_correction_unconfirmed'));
});
