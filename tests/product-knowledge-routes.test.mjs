...
test('admin can process, preview, publish and search document evidence', async () => {
  ...
  try {
    ...
    assert.equal(candidates.payload.summary.count, 1);

    const product = app.db.prepare(`
      SELECT canonical_product_id AS canonicalProductId
      FROM insurance_products
      WHERE tenant_id = 'default'
      LIMIT 1
    `).get();
    const child = processed.payload.chunks.find((chunk) => chunk.chunkType === 'child');
    const bound = await jsonRequest(
      app.baseUrl,
      `/api/admin/product-knowledge/documents/${documentId}/chunks/${child.id}/binding`,
      {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ action: 'bind', canonicalProductId: product.canonicalProductId }),
      },
    );
...
test('review rejects invalid actions and unprocessed publishing', async () => {
  ...
  try {
    ...
    assert.equal(notReady.payload.code, 'PRODUCT_DOCUMENT_BINDING_REQUIRED');
  } finally {
    await app.close();
  }
});
