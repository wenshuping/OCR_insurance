...
const familyRoutesSource = fs.readFileSync(new URL('../server/routes/families.routes.mjs', import.meta.url), 'utf8');
...
test('customer app exposes family profile management surface', () => {
  ...
  assert.match(normalizedCustomerAppSource, /旧版报告，建议重算/);
  ...
  assert.match(familyRoutesSource, /freshness: 'stale', freshnessReason: 'expert_version_changed'/);
  assert.doesNotMatch(familyRoutesSource.match(/function clientSalesReview[\s\S]*?\n  \}/u)?.[0] || '', /structuredResult/);
  assert.doesNotMatch(customerSource, /familySalesReview\?\.content[\s\S]{0,180}资料或专家报告已更新/);
...
test('customer responsibility cards retain the established policy calculation inputs and scenario result path', () => {
  ...
  assert.match(cardSource, /本保单保险金额：/);
  assert.match(cardSource, /有效保险金额/);
  assert.match(cardSource, /本保单首期保费：/);
