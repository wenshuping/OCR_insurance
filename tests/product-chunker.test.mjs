...
function baseChunkInput(overrides = {}) {
  return {
    document: {
      id: 'pdoc_1',
      fileName: '康宁保培训.pptx',
...
test('creates a parent and cited child chunks with deterministic prefixes', () => {
  const chunks = chunkProductDocument(baseChunkInput());
  const parent = chunks.find((chunk) => chunk.chunkType === 'parent');
  const child = chunks.find((chunk) => chunk.chunkType === 'child');
  assert.ok(parent);
  ...
  assert.match(child.contextualPrefix, /资料文件：康宁保培训\.pptx/u);
