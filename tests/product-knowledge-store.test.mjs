...
function bindFirstCandidateChunk(store, tenantId, documentId) {
  const [product] = store.ensureProducts({
    tenantId,
    company: '新华保险',
    productNames: ['康宁保终身重大疾病保险'],
  });
  const review = store.getDocumentIndexReview({ tenantId, documentId });
  const chunk = review.candidateChunks.find((item) => item.chunkType !== 'parent');
  return store.updateCandidateChunkBinding({
    tenantId,
    documentId,
    chunkId: chunk.id,
    action: 'bind',
...
test('search is tenant-scoped and published-only by default', () => {
  ...
  try {
    ...
    bindFirstCandidateChunk(store, 'default', first.document.id);
...
test('product candidates and review decisions are persisted without creating products', () => {
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => error.code === 'PRODUCT_DOCUMENT_CANDIDATE_VERSION_NOT_FOUND',
    );
  } finally {
    db.close();
  }
