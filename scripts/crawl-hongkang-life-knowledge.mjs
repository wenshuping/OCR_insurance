...
function runCrawler(payload) {
  return new Promise((resolve, reject) => {
    ...
    const heartbeat = setInterval(() => {
      ...
      process.stderr.write(`[弘康人寿] crawling ${elapsedSeconds}s...\n`);
    }, 30000);
    ...
    child.on('close', (status) => {
      ...
      if (!line) {
        reject(new Error(`弘康人寿批量爬取没有返回结果\n${stdout}\n${stderr}`));
        return;
      }
      try {
...
async function main() {
  const saleStatus = readArg('sale-status', process.env.HONGKANG_LIFE_SALE_STATUS || 'all');
  const maxProducts = readNumberArg('max-products', Number(process.env.HONGKANG_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.HONGKANG_LIFE_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('弘康人寿'));

    const result = await runCrawler({
    mode: 'hongkang_life_pages',
    company: '弘康人寿',
      skipUrls: [...beforeUrls],
    saleStatus,
    maxProducts,
