...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`东吴人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`东吴人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const maxProducts = readNumberArg('max-products', Number(process.env.SOOCHOW_LIFE_MAX_PRODUCTS || 0));
  const productOffset = readNonNegativeNumberArg('product-offset', Number(process.env.SOOCHOW_LIFE_PRODUCT_OFFSET || 0));
  const maxPages = readNumberArg('max-pages', Number(process.env.SOOCHOW_LIFE_MAX_PAGES || 1));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.SOOCHOW_LIFE_MAX_WORKERS || 6));
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('东吴人寿'));
    const result = runCrawler({
      mode: 'soochow_life_pages',
      company: '东吴人寿',
      maxProducts,
      productOffset,
      maxPages,
