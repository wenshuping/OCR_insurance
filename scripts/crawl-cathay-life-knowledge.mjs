...
function readBooleanArg(name, fallback = false) {
  ...
  const value = String(readArg(name, process.env[name.replace(/-/gu, '_').toUpperCase()] || '')).trim().toLowerCase();
...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`陆家嘴国泰人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`陆家嘴国泰人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

function main() {
  const source = readArg('source', process.env.CATHAY_LIFE_SOURCE || 'product-info');
  ...
  const maxWorkers = readNumberArg('max-workers', Number(process.env.CATHAY_LIFE_MAX_WORKERS || 6));
  const headless = readBooleanArg('headless', false);
  const userDataDir = readArg('user-data-dir', process.env.CATHAY_LIFE_USER_DATA_DIR || '/tmp/chrome-cathaylife-crawl');

  const result = runCrawler({
    mode: source === 'filing' ? 'cathay_life_filing_pages' : 'cathay_life_pages',
    company: '陆家嘴国泰人寿',
    saleStatus,
    maxProducts,
    maxWorkers,
  ...
  if (!result.ok) {
    throw new Error(`陆家嘴国泰人寿爬取未完成：${result.code || ''} ${result.message || ''}`.trim());
  }
  ...
  console.log(
    JSON.stringify(
      {
        ok: true,
        company: result.company || '陆家嘴国泰人寿',
        sourceMode: source,
        saleStatus,
        maxProducts,
        maxWorkers,
