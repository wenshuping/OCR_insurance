...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`众安保险批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`众安保险批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const maxProducts = readNumberArg('max-products', Number(process.env.ZHONGAN_MAX_PRODUCTS || 0));
  const productOffset = readNonNegativeNumberArg('product-offset', Number(process.env.ZHONGAN_PRODUCT_OFFSET || 0));
  const maxPages = readNumberArg('max-pages', Number(process.env.ZHONGAN_MAX_PAGES || 0));
