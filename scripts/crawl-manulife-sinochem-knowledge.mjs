...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`中宏人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`中宏人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.MANULIFE_SINOCHEM_SALE_STATUS || 'all');
  const page = readArg('page', process.env.MANULIFE_SINOCHEM_PAGE || 'all');
  const maxProducts = readNumberArg('max-products', Number(process.env.MANULIFE_SINOCHEM_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.MANULIFE_SINOCHEM_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('中宏人寿'));

    const result = runCrawler({
    mode: 'manulife_sinochem_life_pages',
    company: '中宏人寿',
      skipUrls: [...beforeUrls],
    saleStatus,
    page,
