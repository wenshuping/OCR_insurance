...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`中银三星人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`中银三星人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const startPage = readNumberArg('start-page', Number(process.env.BOC_SAMSUNG_LIFE_START_PAGE || 1));
  const pageSize = readNumberArg('page-size', Number(process.env.BOC_SAMSUNG_LIFE_PAGE_SIZE || 20));
  const maxPages = readNumberArg('max-pages', Number(process.env.BOC_SAMSUNG_LIFE_MAX_PAGES || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.BOC_SAMSUNG_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.BOC_SAMSUNG_LIFE_MAX_WORKERS || 4));
  const newOnly = readBooleanArg('new-only', process.env.BOC_SAMSUNG_LIFE_NEW_ONLY === '1');

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('中银三星人寿'));

    const result = runCrawler({
    mode: 'boc_samsung_life_product_info_pages',
    company: '中银三星人寿',
      skipUrls: [...beforeUrls],
    startPage,
    pageSize,
