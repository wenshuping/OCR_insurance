...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`恒安标准批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`恒安标准批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const source = readArg('source', process.env.HENGANSL_LIFE_SOURCE || 'all');
  const offset = readNumberArg('offset', Number(process.env.HENGANSL_LIFE_OFFSET || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.HENGANSL_LIFE_MAX_PRODUCTS || 0));
  const maxPages = readNumberArg('max-pages', Number(process.env.HENGANSL_LIFE_MAX_PAGES || 0));
  const maxWorkers = Math.max(1, readNumberArg('max-workers', Number(process.env.HENGANSL_LIFE_MAX_WORKERS || 4)));
  const newOnly = readBooleanArg('new-only', process.env.HENGANSL_LIFE_NEW_ONLY === '1');
