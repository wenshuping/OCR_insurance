...
function runCrawler(payload) {
  ...
  if (result.status !== 0) throw new Error(`上海人寿批量爬取失败\n${result.stderr || result.stdout}`);
  const line = String(result.stdout || '').split(/\r?\n/u).reverse().find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`上海人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const maxProducts = readNumberArg('max-products', Number(process.env.SHANGHAI_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.SHANGHAI_LIFE_MAX_WORKERS || 6));
  const pageSize = readNumberArg('page-size', Number(process.env.SHANGHAI_LIFE_PAGE_SIZE || 100));
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const skipUrls = knowledgeStore.knownCompanyUrls(company);
    const result = runCrawler({ mode: 'shanghai_life_pages', company, maxProducts, maxWorkers, pageSize, skipUrls });
    ...
    console.log(JSON.stringify({ ok: true, company, maxProducts, maxWorkers, pageSize, source: result.source, pages: result.pages || [], productCount: (result.products || []).length, materialTaskCount: result.materialTaskCount || 0, knownUrlCount: result.knownUrlCount || 0, skippedExistingUrlCount: result.skippedExistingUrlCount || 0, crawledRecordCount: (result.records || []).length, savedRecordCount: saved.length, newSavedRecordCount: newSaved.length, newSavedMinId: ids[0] || null, newSavedMaxId: ids.at(-1) || null, localKnowledgeBefore: before, localKnowledgeAfter: knowledgeStore.countKnowledgeRecords(), dbPath: knowledgeStore.dbPath }, null, 2));
  } finally {
