...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`珠江人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`珠江人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const source = readArg('source', process.env.PEARL_RIVER_LIFE_SOURCE || 'all');
  const offset = readNumberArg('offset', Number(process.env.PEARL_RIVER_LIFE_OFFSET || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.PEARL_RIVER_LIFE_MAX_PRODUCTS || 0));
  const maxPages = readNumberArg('max-pages', Number(process.env.PEARL_RIVER_LIFE_MAX_PAGES || 0));
  const pageSize = readNumberArg('page-size', Number(process.env.PEARL_RIVER_LIFE_PAGE_SIZE || 500));
  const maxWorkers = Math.max(1, readNumberArg('max-workers', Number(process.env.PEARL_RIVER_LIFE_MAX_WORKERS || 4)));
  const newOnly = readBooleanArg('new-only', process.env.PEARL_RIVER_LIFE_NEW_ONLY === '1');
