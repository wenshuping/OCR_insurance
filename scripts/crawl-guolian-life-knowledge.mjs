...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `国联人寿批量爬取失败 status=${result.status} signal=${result.signal || ''} error=${result.error?.message || ''}\n${stderrTail || stdoutTail}`,
    );
  }
  ...
  if (!line) throw new Error(`国联人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
...
async function main() {
  ...
  const productOffset = readNonNegativeNumberArg('product-offset', Number(process.env.GUOLIAN_LIFE_PRODUCT_OFFSET || 0));
  const maxPages = readNumberArg('max-pages', Number(process.env.GUOLIAN_LIFE_MAX_PAGES || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.GUOLIAN_LIFE_MAX_WORKERS || 4));
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const result = runCrawler({
      mode: 'guolian_life_pages',
      company: '国联人寿',
      saleStatus,
      maxProducts,
      productOffset,
