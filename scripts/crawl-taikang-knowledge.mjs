...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`泰康人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`泰康人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.TAIKANG_LIFE_SALE_STATUS || 'all');
  const maxProducts = readNumberArg('max-products', Number(process.env.TAIKANG_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.TAIKANG_LIFE_MAX_WORKERS || 6));
  const skipExisting = readBooleanArg('skip-existing', true);
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const skipUrls = skipExisting ? knowledgeStore.knownCompanyUrls('泰康人寿') : [];
    const result = runCrawler({
      mode: 'taikang_life_pages', company: '泰康人寿', saleStatus, maxProducts, maxWorkers, skipUrls,
    });
