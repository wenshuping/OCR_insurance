...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`人保寿险批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`人保寿险批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.PICC_LIFE_SALE_STATUS || 'in_sale');
  const startPage = readNumberArg('start-page', Number(process.env.PICC_LIFE_START_PAGE || 1));
  const maxPages = readNumberArg('max-pages', Number(process.env.PICC_LIFE_MAX_PAGES || 0));
  const maxProductsPerPage = readNumberArg('max-products-per-page', Number(process.env.PICC_LIFE_MAX_PRODUCTS_PER_PAGE || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.PICC_LIFE_MAX_WORKERS || 6));
  const maxPageWorkers = readNumberArg('max-page-workers', Number(process.env.PICC_LIFE_MAX_PAGE_WORKERS || 1));
  const skipExisting = readBooleanArg('skip-existing', true);
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const skipUrls = skipExisting ? knowledgeStore.knownCompanyUrls('人保寿险') : [];
    const result = runCrawler({
      mode: 'picc_life_pages', company: '人保寿险', saleStatus, startPage, maxPages,
      maxProductsPerPage, maxWorkers, maxPageWorkers, skipUrls,
    });
    if (result.ok === false) {
    ...
    JSON.stringify(
      {
        ...
        maxWorkers,
        maxPageWorkers,
        detectedTotalPages: result.detectedTotalPages,
        pages: result.pages || [],
