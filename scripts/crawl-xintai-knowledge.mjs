...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`信泰人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`信泰人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.XINTAI_LIFE_SALE_STATUS || 'all');
  const keyword = readArg('keyword', process.env.XINTAI_LIFE_KEYWORD || '');
  const source = readArg('source', process.env.XINTAI_LIFE_SOURCE || 'product-info');
  const maxProducts = readNumberArg('max-products', Number(process.env.XINTAI_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.XINTAI_LIFE_MAX_WORKERS || 6));
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('信泰人寿'));
    const result = runCrawler({
      mode: source === 'internet' ? 'xintai_life_internet_products' : 'xintai_life_product_info',
      company: '信泰人寿',
      saleStatus,
      keyword,
      maxProducts,
