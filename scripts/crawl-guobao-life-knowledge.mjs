...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`国宝人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`国宝人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
...
async function main() {
  ...
  const maxProducts = readNumberArg('max-products', Number(process.env.GUOBAO_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.GUOBAO_LIFE_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('国宝人寿'));
