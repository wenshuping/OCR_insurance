...
function knownCompanyProductNames(state, company) {
  return [
    ...new Set(
      (state.knowledgeRecords || [])
        .filter((record) => String(record.company || '').trim() === company)
        .map((record) => String(record.productName || '').trim())
        .filter(Boolean),
    ),
  ];
}
...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`新华保险批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`新华保险批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

function main() {
  const pageBase = readArg('page-base', process.env.NEW_CHINA_PAGE_BASE || '4596');
  const startPage = readNumberArg('start-page', Number(process.env.NEW_CHINA_START_PAGE || 1));
  const maxPages = readNumberArg('max-pages', Number(process.env.NEW_CHINA_MAX_PAGES || 1));
  const maxProductsPerPage = readNumberArg('max-products-per-page', Number(process.env.NEW_CHINA_MAX_PRODUCTS_PER_PAGE || 0));
  const skipExisting = readBooleanArg('skip-existing', true);
  const skipState = readJson(statePath, createInitialState());
  const skipUrls = skipExisting ? knownCompanyUrls(skipState, '新华保险') : [];
  const skipProductNames = skipExisting ? knownCompanyProductNames(skipState, '新华保险') : [];

  const result = runCrawler({
    mode: 'new_china_pages',
    company: '新华保险',
    pageBase,
    startPage,
    maxPages,
    ...
    JSON.stringify(
      {
        ...
        skippedExistingProductNameCount: skipProductNames.length,
