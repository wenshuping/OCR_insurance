...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`光大永明批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`光大永明批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.SUNLIFE_EVERBRIGHT_SALE_STATUS || 'all');
  const offset = Math.max(
    0,
    Number(readArg('offset', readArg('product-offset', process.env.SUNLIFE_EVERBRIGHT_PRODUCT_OFFSET || '0'))) || 0,
  );
  const maxProducts = readNumberArg('max-products', Number(process.env.SUNLIFE_EVERBRIGHT_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.SUNLIFE_EVERBRIGHT_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('光大永明人寿'));

    const result = runCrawler({
    mode: 'sunlife_everbright_life_pages',
    company: '光大永明人寿',
      skipUrls: [...beforeUrls],
    saleStatus,
    offset,
