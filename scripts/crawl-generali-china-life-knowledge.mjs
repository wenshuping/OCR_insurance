...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`中意人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`中意人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.GENERALI_CHINA_LIFE_SALE_STATUS || 'all');
  const segment = readArg('segment', process.env.GENERALI_CHINA_LIFE_SEGMENT || 'all');
  const offset = Math.max(0, Number(readArg('offset', process.env.GENERALI_CHINA_LIFE_OFFSET || 0)) || 0);
  const maxProducts = readNumberArg('max-products', Number(process.env.GENERALI_CHINA_LIFE_MAX_PRODUCTS || 0));
  const maxPages = readNumberArg('max-pages', Number(process.env.GENERALI_CHINA_LIFE_MAX_PAGES || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.GENERALI_CHINA_LIFE_MAX_WORKERS || 6));
  const maxDetailWorkers = readNumberArg('max-detail-workers', Number(process.env.GENERALI_CHINA_LIFE_MAX_DETAIL_WORKERS || maxWorkers));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('中意人寿'));
    const result = runCrawler({
      mode: 'generali_china_life_pages',
      company: '中意人寿',
      saleStatus,
      segment,
      offset,
