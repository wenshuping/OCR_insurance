...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`工银安盛批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`工银安盛批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const maxProducts = readNumberArg('max-products', Number(process.env.ICBC_AXA_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.ICBC_AXA_MAX_WORKERS || 6));
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const result = runCrawler({
      mode: 'icbc_axa_life_pages',
      company: '工银安盛',
      maxProducts,
      maxWorkers,
    });
