...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`北大方正人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`北大方正人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const maxProducts = readNumberArg('max-products', Number(process.env.PKU_FOUNDER_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.PKU_FOUNDER_LIFE_MAX_WORKERS || 2));
  const offset = readNumberArg('offset', Number(process.env.PKU_FOUNDER_LIFE_OFFSET || 0));
  const saleStatus = readArg('sale-status', process.env.PKU_FOUNDER_LIFE_SALE_STATUS || 'all');
  const sourceScope = readArg('source', process.env.PKU_FOUNDER_LIFE_SOURCE || 'all');
  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('北大方正人寿'));

    const result = runCrawler({
    mode: 'pku_founder_life_pages',
    company: '北大方正人寿',
      skipUrls: [...beforeUrls],
    maxProducts,
    maxWorkers,
