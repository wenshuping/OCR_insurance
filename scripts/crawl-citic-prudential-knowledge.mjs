...
function readOffsetArg(name, fallback) {
  const value = Number(readArg(name, ''));
  return Number.isFinite(value) && value >= 0 ? value : fallback;
}
...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`中信保诚批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`中信保诚批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.CITIC_PRUDENTIAL_SALE_STATUS || 'all');
  const offset = readOffsetArg('offset', Number(process.env.CITIC_PRUDENTIAL_OFFSET || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.CITIC_PRUDENTIAL_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.CITIC_PRUDENTIAL_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('中信保诚人寿'));

    const result = runCrawler({
    mode: 'citic_prudential_life_pages',
    company: '中信保诚人寿',
      skipUrls: [...beforeUrls],
    saleStatus,
    offset,
