...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`富德生命批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`富德生命批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const source = readArg('source', process.env.FUNDE_LIFE_SOURCE || 'all');
  const offset = readNumberArg('offset', Number(process.env.FUNDE_LIFE_OFFSET || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.FUNDE_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = Math.max(1, readNumberArg('max-workers', Number(process.env.FUNDE_LIFE_MAX_WORKERS || 4)));
  const newOnly = readBooleanArg('new-only', process.env.FUNDE_LIFE_NEW_ONLY === '1');

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('富德生命'));
    const result = runCrawler({
      mode: 'funde_life_pages',
      company: '富德生命',
      source,
      offset,
      maxProducts,
