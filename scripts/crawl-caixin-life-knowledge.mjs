...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`财信人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`财信人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.CAIXIN_LIFE_SALE_STATUS || 'all');
  const startIndex = readNumberArg('start-index', Number(process.env.CAIXIN_LIFE_START_INDEX || 1));
  const maxProducts = readNumberArg('max-products', Number(process.env.CAIXIN_LIFE_MAX_PRODUCTS || 0));
