...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`合众人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`合众人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

function main() {
  const sourceScope = readArg('source-scope', process.env.UNION_LIFE_SOURCE_SCOPE || 'all');
  const maxProducts = readNumberArg('max-products', Number(process.env.UNION_LIFE_MAX_PRODUCTS || 0));
