...
function readArtifactRows(filePath) {
...
export function prepareReviewedResponsibilityArtifacts({ artifacts = [] } = {}) {
  return artifacts.flatMap((artifact) => readArtifactRows(path.resolve(artifact)))
    .map(normalizeSemanticProduct);
}
...
function indicatorFrom(product = {}, responsibility = {}, now = new Date().toISOString()) {
  ...
  const base = {
    ...
    sourceTitle,
    sourceExcerpt,
    sourceDigest: text(check.sourceDigest || sourceRecord.sourceDigest || product.sourceDigest),
  ...
  return {
    ...
    requiredInputs: rows(check.requiredInputs).length ? rows(check.requiredInputs) : calculatedFields.requiredInputs,
  };
}
...
export function importReviewedResponsibilityArtifacts({
  ...
  const products = prepareReviewedResponsibilityArtifacts({ artifacts });
  const samples = [];
  const blockers = [];
