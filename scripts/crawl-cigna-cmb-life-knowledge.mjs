...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`招商信诺批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`招商信诺批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const source = readArg('source', process.env.CIGNA_CMB_LIFE_SOURCE || 'all');
  const offset = readNumberArg('offset', Number(process.env.CIGNA_CMB_LIFE_OFFSET || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.CIGNA_CMB_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = Math.max(1, readNumberArg('max-workers', Number(process.env.CIGNA_CMB_LIFE_MAX_WORKERS || 4)));
  const newOnly = readBooleanArg('new-only', process.env.CIGNA_CMB_LIFE_NEW_ONLY === '1');

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const beforeUrls = new Set(knowledgeStore.knownCompanyUrls('招商信诺'));
    const result = runCrawler({
      mode: 'cigna_cmb_life_pages',
      company: '招商信诺',
      source,
      offset,
      maxProducts,
