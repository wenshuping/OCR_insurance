...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`同方全球批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`同方全球批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.AEGON_THTF_SALE_STATUS || 'all');
  const page = readArg('page', process.env.AEGON_THTF_PAGE || 'all');
  const offset = readNumberArg('offset', Number(process.env.AEGON_THTF_OFFSET || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.AEGON_THTF_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.AEGON_THTF_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const result = runCrawler({
      mode: 'aegon_thtf_life_pages',
      company: '同方全球人寿',
      saleStatus,
      page,
      offset,
