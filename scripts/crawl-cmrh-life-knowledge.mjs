...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`招商仁和批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`招商仁和批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const saleStatus = readArg('sale-status', process.env.CMRH_LIFE_SALE_STATUS || 'all');
  const pageSize = readNumberArg('page-size', Number(process.env.CMRH_LIFE_PAGE_SIZE || 100));
  const startPage = readNumberArg('start-page', Number(process.env.CMRH_LIFE_START_PAGE || 0));
  const endPage = readNumberArg('end-page', Number(process.env.CMRH_LIFE_END_PAGE || 0));
  const maxProducts = readNumberArg('max-products', Number(process.env.CMRH_LIFE_MAX_PRODUCTS || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.CMRH_LIFE_MAX_WORKERS || 6));

  const knowledgeStore = await createKnowledgeStateStore();
  try {
    const skipUrls = knowledgeStore.knownCompanyUrls('招商仁和');
    const result = runCrawler({
      mode: 'cmrh_life_pages', company: '招商仁和', saleStatus, pageSize,
      startPage, endPage, maxProducts, maxWorkers, skipUrls,
    });
