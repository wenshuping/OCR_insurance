...
function runCrawler(payload) {
  ...
  if (result.status !== 0) {
    throw new Error(`小康人寿批量爬取失败\n${result.stderr || result.stdout}`);
  }
  const line = String(result.stdout || '')
    .split(/\r?\n/u)
    .reverse()
    .find((item) => item.includes(outputMarker));
  if (!line) throw new Error(`小康人寿批量爬取没有返回结果\n${result.stdout}\n${result.stderr}`);
  return JSON.parse(line.slice(line.indexOf(outputMarker) + outputMarker.length));
}

async function main() {
  const maxProducts = readNumberArg('max-products', Number(process.env.XIAOKANG_LIFE_MAX_PRODUCTS || 0));
  const offset = readNonNegativeNumberArg('offset', Number(process.env.XIAOKANG_LIFE_OFFSET || 0));
  const maxWorkers = readNumberArg('max-workers', Number(process.env.XIAOKANG_LIFE_MAX_WORKERS || 4));
  const saleStatus = readArg('sale-status', process.env.XIAOKANG_LIFE_SALE_STATUS || 'all');
