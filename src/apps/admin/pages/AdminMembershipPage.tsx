  ...
  return (
    <section className="max-w-xl rounded-[18px] border border-slate-200 bg-white p-5 shadow-[0_20px_60px_-46px_rgba(15,23,42,0.42)]">
      <div className="mb-5 flex items-start justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          <h2 className="text-lg font-black text-slate-950">会员与报告重算设置</h2>
          <p className="mt-1 text-sm font-semibold text-slate-500">控制会员购买、免费保单额度和用户每日主动重算次数</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="block">
          <span className="text-xs font-black text-slate-400">家庭保单分析报告每日重算次数</span>
          <input
            className="mt-1 h-11 w-full rounded-xl border border-blue-100 bg-white px-3 text-sm font-bold text-slate-900 outline-none transition focus:border-blue-400"
            type="number"
            min="0"
        ...
        <label className="block">
          <span className="text-xs font-black text-slate-400">营销建议报告每日重算次数</span>
          <input
            className="mt-1 h-11 w-full rounded-xl border border-blue-100 bg-white px-3 text-sm font-bold text-slate-900 outline-none transition focus:border-blue-400"
            type="number"
            min="0"
      ...
      <p className="mt-3 text-xs font-semibold leading-5 text-slate-500">重算次数只统计用户在前台主动点击重新生成，不统计后台查看、后台自动修正或系统自动生成。</p>
