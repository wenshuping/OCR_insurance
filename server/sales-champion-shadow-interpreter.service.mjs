...
const RULES = Object.freeze([
  ...
  { pattern: /(?:理赔|赔不赔|免责|等待期|核保|健康告知|收益|保额|条款|续保)/u, concern: 'benefits', capability: 'plain_language_explanation', stage: 'proposal', factSensitive: true },
...
export function interpretSalesChampionShadowTurn({ question } = {}) {
  ...
  const factSensitive = RULES.some((candidate) => candidate.factSensitive === true && candidate.pattern.test(current));
  const rule = matched || {
    concern: 'unknown',
