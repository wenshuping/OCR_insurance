...
const INSURER_OFFICIAL_PROFILES = [
  ...
  },
  {
    ...
    officialDomains: ['citic-prudential.com.cn', 'gwoss.citic-prudential.citic'],
  },
  {
    id: 'hsbc_life_china',
    aliases: ['汇丰', '汇丰人寿'],
  ...
  },
  {
    ...
    siteDomains: ['ydthlife.com'],
    officialDomains: ['ydthlife.com'],
  },
  {
    id: 'guolian_life',
    aliases: ['国联', '国联人寿'],
    companyAliases: ['国联人寿', '国联人寿保险股份有限公司'],
    siteDomains: ['guolian-life.com'],
    officialDomains: ['guolian-life.com'],
  },
  {
    id: 'pku_founder_life',
    aliases: ['北大方正', '方正人寿'],
    companyAliases: ['北大方正人寿', '北大方正人寿保险有限公司'],
    siteDomains: ['pkufi.com'],
    officialDomains: ['pkufi.com'],
  },
  {
    id: 'aegon_thtf_life',
    aliases: ['同方全球', '同方全球人寿'],
  ...
  },
  {
    ...
    siteDomains: ['bobl.com.cn'],
    officialDomains: ['bobl.com.cn'],
  },
  {
    id: 'hengansl_life',
    aliases: ['恒安标准', '恒安标准人寿'],
  ...
  },
  {
    ...
    siteDomains: ['hqins.cn'],
    officialDomains: ['hqins.cn'],
  },
  {
    id: 'hongkang_life',
    aliases: ['弘康', '弘康人寿'],
  ...
  },
  {
    ...
    siteDomains: ['prudentialfosun.com.cn'],
    officialDomains: ['prudentialfosun.com.cn'],
  },
  {
  ...
  },
  {
    ...
    siteDomains: ['blife.com.cn'],
    officialDomains: ['blife.com.cn'],
  },
  {
    id: 'dingcheng_life',
    aliases: ['鼎诚', '鼎诚人寿'],
    companyAliases: ['鼎诚人寿', '鼎诚人寿保险有限责任公司'],
    siteDomains: ['dc-life.com.cn'],
    officialDomains: ['dc-life.com.cn'],
  },
  {
    id: 'boc_samsung_life',
    aliases: ['中银三星', '中银三星人寿'],
  ...
  },
  {
    ...
    siteDomains: ['soochowlife.com'],
    officialDomains: ['soochowlife.com'],
  },
  {
    id: 'bohai_life',
    aliases: ['渤海', '渤海人寿'],
...
function buildParticipantSummary(policy = {}) {
  ...
  if (applicantRelation && insuredRelation && applicantRelation === insuredRelation) {
    return `投保人与被保险人与录入人的关系均为${applicantRelation}`;
  }
  const parts = [];
  if (applicantRelation) parts.push(`投保人与录入人的关系为${applicantRelation}`);
  if (insuredRelation) parts.push(`被保险人与录入人的关系为${insuredRelation}`);
  return parts.join('；');
}
  ...
  return [
    {
      role: 'system',
      content:
        '你是保险条款责任提炼助手。你只能基于用户提供的条款页/OCR文本，对已有责任表进行二次细化。不要重写产品概览，不要输出注意事项，只输出 JSON。JSON 只能包含 coverageTable 一个字段。coverageTable 只能写发生保险事故、达到领取条件或满足合同约定触发条件后的保险金/给付/赔付/报销责任，每行必须包含 coverageType、scenario、payout、note。分红、红利领取方式、现金价值、减保、保单贷款、自动垫交等产品利益机制不要作为 coverageTable 独立行；若有效保额递增或给付比例属于某条保险责任的计算公式，则写进该责任的 payout。若 OCR 中能明确识别责任触发情形、给付规则、给付比例、满期返还条件、赔付倍数，就直接写明，不要再用“需以正式条款核对”兜底。note 必须用面向客户的解释性中文，不要只写“合同终止”“责任终止”“合同中止”这种术语标签，必须把它解释成这对客户意味着什么。只有在 OCR 文本确实没有给出该责任的关键内容时，才保留核对提醒。',
    },
    {
      role: 'user',
...
function normalizeAnalysis(payload, model, options = {}) {
  ...
  return {
    ...
    productOverview: '',
    coreFeature: '',
    coverageTable: parsed.coverageTable.slice(0, 12).map((row) => ({
      coverageType: removeSourceDisclosureText(redactSensitiveAnalysisText(row.coverageType, sensitiveTerms)),
      scenario: removeSourceDisclosureText(redactSensitiveAnalysisText(row.scenario, sensitiveTerms)),
      payout: removeSourceDisclosureText(redactSensitiveAnalysisText(row.payout, sensitiveTerms)),
      note: removeSourceDisclosureText(redactSensitiveAnalysisText(row.note, sensitiveTerms)),
    })),
    notes: [],
    purchaseAdvice: '',
    disclaimer: '',
...
3. coverageTable 是保险责任表，只能写保险公司在“发生保险事故、达到领取条件或满足合同约定触发条件”后承担的给付/赔付/报销责任。每一条保险责任都要单独写入 coverageTable 的一行；分阶段给付、额外给付、不同领取条件也要拆成独立行，不要把多项责任合并成一行。每行必须包含 coverageType、scenario、payout、note。给付比例、等待期、领取年龄、赔付倍数、保额递增公式等如果属于该责任的给付计算口径，必须写明。
4. 分红、红利领取方式、现金价值、减保、保单贷款、自动垫交、转换年金等属于产品利益机制或保全权益，不要作为 coverageTable 的独立行；若有效保额递增、现金价值或给付比例属于某条保险责任的计算公式，则只写进该责任的 payout。
5. 上下文没有明确证据的内容不要编造；不要输出“本产品未明确提及某某责任”来凑内容。若产品资料和 OCR 不一致，保险责任以正式条款口径为主，个单金额、保费、期间以 OCR 或结构化保单字段为主。
...
export async function analyzeInsurancePolicyResponsibilities({
  ...
  const searchArtifacts = hasOfficialSearchSource(knowledgeArtifacts.sources)
    ? knowledgeArtifacts
  ...
  if (config.smartSearchEnabled && searchQuery && !hasOfficialSearchSource(searchArtifacts.sources)) {
    throw withCode(
