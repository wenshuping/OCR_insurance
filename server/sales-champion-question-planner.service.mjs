...
const SLOT_DEFINITIONS = Object.freeze({
  customer_relationship_origin: Object.freeze({
    question: '这是你以前一直服务的客户，还是最近接手、转介绍或公司分配的？',
    cost: 1,
    priority: 90,
  ...
  explicit_customer_request: Object.freeze({
    question: '客户这次明确希望你帮他解决什么？',
    cost: 1,
    priority: 85,
    targetLabels: ['demand_maturity', 'next_action'],
  }),
...
export function planSalesChampionQuestions({
  ...
  const slots = [...new Set([
    ...affectedSkills.keys(),
    ...(Array.isArray(proposal?.missingInformation) ? proposal.missingInformation : []),
  ])];
  ...
  const ranked = slots.flatMap((slot) => {
    ...
    return [{ slot, definition, affectedSkills: [...new Set(affectedSkills.get(slot) || [])] }];
  }).sort((left, right) => (
