  ...
  return (Array.isArray(source) ? source : [])
    .map((item) => {
      ...
      try {
        ...
        raw = { memoryKey: item.memoryKey ?? item.memory_key, normalizedValue: item.normalizedValue ?? item.normalized_value, validFrom: item.validFrom ?? item.valid_from };
        for (const [key, limit] of [['kind', 32], ['memoryKey', 120], ['content', 2_000], ['text', 2_000], ['summary', 2_000], ['normalizedValue', 500], ['validFrom', 64]]) {
          const field = raw[key] ?? item[key];
      ...
      return {
        ...
        content,
        confidence,
        memoryKey: normalizeProvidedMemoryKey(kind, raw.memoryKey, content),
        normalizedValue: sanitizeMemoryContent(raw.normalizedValue || content),
        sourceType: 'user_statement',
        validFrom: normalizeIso(raw.validFrom),
      };
    })
    .filter(Boolean)
  ...
  for (const item of normalized) {
    ...
    if (existing) {
      ...
      if (existingValue === item.normalizedValue) {
        ...
        if (Number.isSafeInteger(eventSourceMessageId) && eventSourceMessageId > 0
          && previousEvidenceMessageIds.includes(eventSourceMessageId)) continue;
        ...
        existing.sourceMessageIds = existing.evidenceMessageIds;
        existing.sourceMessageId = existing.evidenceMessageIds[0] || null;
        ...
        changes.push({
          kind: 'reinforced',
          memory: { ...existing, evidenceMessageIds: [...existing.evidenceMessageIds], sourceMessageIds: [...existing.sourceMessageIds] },
          expectedVersion: previousVersion,
          previousEvidenceMessageIds,
          addedEvidenceMessageIds: existing.evidenceMessageIds.filter((id) => !previousEvidenceMessageIds.includes(id)),
          eventSourceMessageId: Number.isSafeInteger(eventSourceMessageId) && eventSourceMessageId > 0 ? eventSourceMessageId : null,
        });
        continue;
      }
      const previousVersion = Number(existing.version || 1);
      ...
      existing.updatedAt = now;
      changedMemories.push(existing);
      changes.push({ kind: 'conflicted', memory: { ...existing }, expectedVersion: previousVersion, previousStatus: 'confirmed' });
    }
    ...
    const memory = {
      ...
      version: 1,
      validFrom: autoConfirmed ? (item.validFrom || now) : null,
      validTo: null,
