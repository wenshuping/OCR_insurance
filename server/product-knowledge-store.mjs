...
export function createProductKnowledgeStore(db) {
  ...
  function replaceParsedArtifacts(input = {}) {
    ...
    db.exec('BEGIN IMMEDIATE');
    try {
      db.prepare('DELETE FROM knowledge_chunks_fts WHERE tenant_id = ? AND document_id = ?').run(tenantId, documentId);
      ...
      db.prepare(`
        UPDATE product_documents
        SET document_type = ?, parse_status = 'indexed_pending_review', updated_at = ?, payload = ?
        WHERE tenant_id = ? AND id = ?
      `).run(text(input.documentType) || document.documentType, now, jsonPayload(documentPayload), tenantId, documentId);
      db.exec('COMMIT');
    } catch (error) {
      db.exec('ROLLBACK');
      throw error;
  ...
  function listDocumentChunks({ tenantId, documentId } = {}) {
    return db.prepare(`
  ...
  function reviewDocument(input = {}) {
    ...
    if (!['publish', 'reject'].includes(action)) {
      const error = new Error('审核动作必须是publish或reject');
    ...
    if (action === 'publish') {
      const count = db.prepare(`
        ...
        WHERE tenant_id = ? AND document_id = ? AND chunk_type != 'parent'
      `).get(tenantId, documentId)?.count || 0;
      if (!count) {
    ...
    const now = text(input.now) || new Date().toISOString();
    const reviewStatus = action === 'publish' ? 'published' : 'rejected';
    const payload = {
      ...document.payload,
    ...
    try {
      ...
      db.prepare(`
        UPDATE knowledge_chunks SET review_status = ?, updated_at = ?
        WHERE tenant_id = ? AND document_id = ?
      `).run(reviewStatus, now, tenantId, documentId);
      db.exec('COMMIT');
    } catch (error) {
      db.exec('ROLLBACK');
      throw error;
  ...
  function searchChunks(input = {}) {
    ...
    const conditions = ['chunks.tenant_id = ?', "chunks.chunk_type != 'parent'"];
