export function createAgentReportRegenerationQueue({ state, workflow, familyOwnerMatches } = {}) {
  ...
  return {
    async enqueueUnique({ familyId, type, userId, dedupeKey } = {}) {
      ...
      const sourceUpdatedAt = [family, ...(state.familyMembers || []), ...(state.policies || [])]
      ...
      const collection = type === 'family_report' ? state.familyReports : state.familySalesReviews;
      ...
      if (type === 'family_report') await workflow.regenerateCoverage({ family, owner, system: true });
      else if (type === 'family_sales_review') await workflow.regenerateSalesReview({ family, owner, system: true });
